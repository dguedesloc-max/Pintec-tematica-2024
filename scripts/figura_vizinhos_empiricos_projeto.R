# =============================================================================
# Figura - Objetivo (iv), apoio: CNAE 26 e vizinhos empíricos mais próximos
# Par Desenvolvimento de projetos - Produção, IA e Cloud
# (mesmo critério de seleção da figura equivalente para Admin-Produção:
#  percentil extremo + CV confiável)
# =============================================================================
...
vizinhos <- read.csv(
  here("data", "processed", "vizinhos_cnae26_projeto_producao.csv"),
  stringsAsFactors = FALSE
) %>%
  filter(tecnologia %in% c("IA", "Cloud")) %>%
  mutate(
    rotulo = paste0("CNAE ", codigo_cnae),
    tecnologia = factor(tecnologia, levels = c("IA", "Cloud")),
    aviso_cv = factor(aviso_cv, levels = c("OK", "CV alto", "Sem dado")),
    # chave interna única por painel, para evitar que um mesmo rótulo
    # (ex.: "CNAE 26", presente nos dois painéis) herde a ordem de um
    # painel só -- cada painel ordena de forma independente
...
grafico_vizinhos_projeto <- ggplot(vizinhos, aes(x = rotulo_interno, y = asimetria_pp)) +
  ...
  labs(
    ...
    subtitle = "Assimetria Desenvolvimento de projetos\u2212Produção (p.p.). Vizinhos = 2 divisões\nacima e abaixo de CNAE 26 no ranking das 24 divisões CNAE da indústria.",
    x = NULL,
    y = "Assimetria Projeto\u2212Produção (p.p.)"
  ) +
...
print(grafico_vizinhos_projeto)
...
ggsave(
  here("output", "figura_vizinhos_empiricos_projeto_producao.png"),
  grafico_vizinhos_projeto,
